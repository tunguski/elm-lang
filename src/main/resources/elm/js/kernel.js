...
var $rt = {
  ...
  'Dict.foldl': function(f){ return function(acc0){ return function(d){ var acc=acc0; d.a.forEach(function(e){ acc=A3(f,e[0],e[1],acc); }); return acc; }; }; },
  'Dict.foldr': function(f){ return function(acc0){ return function(d){ var acc=acc0; for(var i=d.a.length-1;i>=0;i--) acc=A3(f,d.a[i][0],d.a[i][1],acc); return acc; }; }; },
  'Dict.map': function(f){ return function(d){ return {$:'Dict',a:d.a.map(function(e){return [e[0],A2(f,e[0],e[1])];})}; }; },
  'Dict.filter': function(f){ return function(d){ return {$:'Dict',a:d.a.filter(function(e){return A2(f,e[0],e[1]);})}; }; },
  ...
  'Dict.partition': function(f){ return function(d){ var yes=[],no=[]; d.a.forEach(function(e){ (A2(f,e[0],e[1])?yes:no).push(e); }); return $tuple([{$:'Dict',a:yes},{$:'Dict',a:no}]); }; },
  'Dict.merge': function(left){ return function(both){ return function(right){ return function(la){ return function(rb){ return function(acc){ var L=la.a,R=rb.a,i=0,j=0;
    while(i<L.length&&j<R.length){ var c=$cmp(L[i][0],R[j][0]); if(c<0){acc=A3(left,L[i][0],L[i][1],acc);i++;} else if(c>0){acc=A3(right,R[j][0],R[j][1],acc);j++;} else {acc=A4(both,L[i][0],L[i][1],R[j][1],acc);i++;j++;} }
    for(;i<L.length;i++)acc=A3(left,L[i][0],L[i][1],acc); for(;j<R.length;j++)acc=A3(right,R[j][0],R[j][1],acc); return acc; }; }; }; }; }; },
  'Set.empty': {$:'Set',a:[]},
  ...
  'Set.foldl': function(f){ return function(acc0){ return function(s){ var acc=acc0; s.a.forEach(function(x){ acc=A2(f,x,acc); }); return acc; }; }; },
  'Set.foldr': function(f){ return function(acc0){ return function(s){ var acc=acc0; for(var i=s.a.length-1;i>=0;i--) acc=A2(f,s.a[i],acc); return acc; }; }; },
  ...
  'Array.indexedMap': function(f){ return function(arr){ return {$:'Array',a:arr.a.map(function(x,i){return A2(f,i,x);})}; }; },
  'Array.foldl': function(f){ return function(acc0){ return function(arr){ var acc=acc0; arr.a.forEach(function(x){ acc=A2(f,x,acc); }); return acc; }; }; },
  'Array.foldr': function(f){ return function(acc0){ return function(arr){ var acc=acc0; for(var i=arr.a.length-1;i>=0;i--) acc=A2(f,arr.a[i],acc); return acc; }; }; },
  ...
  'List.indexedMap': function(f){ return function(xs){ var o=[],i=0; while(xs.$==='::'){ o.push(A2(f,i,xs.a)); i++; xs=xs.b; } return $list(o); }; },
  'List.filter': function(f){ return function(xs){ var o=[]; while(xs.$==='::'){ if(f(xs.a)) o.push(xs.a); xs=xs.b; } return $list(o); }; },
  // foldl/foldr copy the captured initial accumulator into a fresh local before looping: a point-free
  // partial application (e.g. `distinct = List.foldl f []`, `sum = List.foldl (+) 0`) is a single shared
  // closure, so mutating the captured `acc0` would leak state from one call into the next.
  'List.foldl': function(f){ return function(acc0){ return function(xs){ var acc=acc0; while(xs.$==='::'){ acc=A2(f,xs.a,acc); xs=xs.b; } return acc; }; }; },
  'List.foldr': function(f){ return function(acc0){ return function(xs){ var acc=acc0, a=$listToArray(xs); for(var i=a.length-1;i>=0;i--) acc=A2(f,a[i],acc); return acc; }; }; },
  ...
  'List.map3': function(f){ return function(a){ return function(b){ return function(c){ var aa=$listToArray(a),bb=$listToArray(b),cc=$listToArray(c),n=Math.min(aa.length,bb.length,cc.length),o=[]; for(var i=0;i<n;i++) o.push(A3(f,aa[i],bb[i],cc[i])); return $list(o); }; }; }; },
  'List.map4': function(f){ return function(a){ return function(b){ return function(c){ return function(d){ var aa=$listToArray(a),bb=$listToArray(b),cc=$listToArray(c),dd=$listToArray(d),n=Math.min(aa.length,bb.length,cc.length,dd.length),o=[]; for(var i=0;i<n;i++) o.push(A4(f,aa[i],bb[i],cc[i],dd[i])); return $list(o); }; }; }; }; },
  'List.map5': function(f){ return function(a){ return function(b){ return function(c){ return function(d){ return function(e){ var aa=$listToArray(a),bb=$listToArray(b),cc=$listToArray(c),dd=$listToArray(d),ee=$listToArray(e),n=Math.min(aa.length,bb.length,cc.length,dd.length,ee.length),o=[]; for(var i=0;i<n;i++) o.push(A5(f,aa[i],bb[i],cc[i],dd[i],ee[i])); return $list(o); }; }; }; }; }; },
  ...
  'List.sortWith': function(f){ return function(xs){ return $list($listToArray(xs).slice().sort(function(a,b){ return $ordToInt(A2(f,a,b)); })); }; },
  ...
  'List.map2': function(f){ return function(xs){ return function(ys){ var a=$listToArray(xs), b=$listToArray(ys), o=[]; for(var i=0;i<Math.min(a.length,b.length);i++) o.push(A2(f,a[i],b[i])); return $list(o); }; }; },
  ...
  'String.foldl': function(f){ return function(acc0){ return function(s){ var acc=acc0; Array.from(s).forEach(function(ch){ acc=A2(f,$char(ch.codePointAt(0)),acc); }); return acc; }; }; },
  'String.foldr': function(f){ return function(acc0){ return function(s){ var acc=acc0, a=Array.from(s); for(var i=a.length-1;i>=0;i--) acc=A2(f,$char(a[i].codePointAt(0)),acc); return acc; }; }; },
  ...
  'Maybe.map2': function(f){ return function(a){ return function(b){ return (a.$==='Just'&&b.$==='Just')?$data('Just',[A2(f,a._[0],b._[0])]):$data('Nothing',[]); }; }; },
  'Maybe.map3': function(f){ return function(a){ return function(b){ return function(c){ return (a.$==='Just'&&b.$==='Just'&&c.$==='Just')?$data('Just',[A3(f,a._[0],b._[0],c._[0])]):$data('Nothing',[]); }; }; }; },
  'Maybe.map4': function(f){ return function(a){ return function(b){ return function(c){ return function(d){ return (a.$==='Just'&&b.$==='Just'&&c.$==='Just'&&d.$==='Just')?$data('Just',[A4(f,a._[0],b._[0],c._[0],d._[0])]):$data('Nothing',[]); }; }; }; }; },
  'Maybe.map5': function(f){ return function(a){ return function(b){ return function(c){ return function(d){ return function(e){ return (a.$==='Just'&&b.$==='Just'&&c.$==='Just'&&d.$==='Just'&&e.$==='Just')?$data('Just',[A5(f,a._[0],b._[0],c._[0],d._[0],e._[0])]):$data('Nothing',[]); }; }; }; }; }; },
  ...
  'Result.map2': function(f){ return function(a){ return function(b){ if(a.$==='Err')return a; if(b.$==='Err')return b; return $data('Ok',[A2(f,a._[0],b._[0])]); }; }; },
  'Result.map3': function(f){ return function(a){ return function(b){ return function(c){ if(a.$==='Err')return a; if(b.$==='Err')return b; if(c.$==='Err')return c; return $data('Ok',[A3(f,a._[0],b._[0],c._[0])]); }; }; }; },
  'Result.map4': function(f){ return function(a){ return function(b){ return function(c){ return function(d){ if(a.$==='Err')return a; if(b.$==='Err')return b; if(c.$==='Err')return c; if(d.$==='Err')return d; return $data('Ok',[A4(f,a._[0],b._[0],c._[0],d._[0])]); }; }; }; }; },
  'Result.map5': function(f){ return function(a){ return function(b){ return function(c){ return function(d){ return function(e){ if(a.$==='Err')return a; if(b.$==='Err')return b; if(c.$==='Err')return c; if(d.$==='Err')return d; if(e.$==='Err')return e; return $data('Ok',[A5(f,a._[0],b._[0],c._[0],d._[0],e._[0])]); }; }; }; }; }; },
