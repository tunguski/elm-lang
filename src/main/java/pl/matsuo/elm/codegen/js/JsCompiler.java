...
public final class JsCompiler {
   ...
   * quotes; {@link #jsString} emits double). So an entry whose {@code "Module.name"} never appears is
   * unreachable and removed. Conservative: only self-contained (brace/paren-balanced) lines are
   * dropped, and the double-quoted form never collides with a definition or a longer name.
   */
