 ...
 *
 * <p>A small <b>standard library</b> ({@link WasmPrelude}) — {@code List.map}/{@code foldl}/{@code
...
public final class WasmCompiler {
  ...
  static final int I32 = 0x7F, I64 = 0x7E;
  ...
  private static byte[] compileModules(List<pl.matsuo.elm.ast.Module> modules, boolean wantPrelude) {
    ...
    if (wantPrelude) {
      decls.addAll(pl.matsuo.elm.parser.Parser.parseModule(WasmPrelude.SOURCE).decls());
    }
    for (Decl d : decls) {
  ...
  private static final class FunctionGen {
    ...
    private String resolvedName(Expr.Var v) {
      if (v.module() != null) {
        ...
        if (WasmPrelude.NAMES.containsKey(qualified)) {
          return WasmPrelude.NAMES.get(qualified);
        }
      }
      return v.name();
    }
  ...
  record Native(String name, int arity, byte[] entry) {}

  static byte[] entry(ByteArrayOutputStream body, int[][] localGroups) {
  ...
  private static byte[] assemble(
    ...
    List<Native> natives = new ArrayList<>(WasmNativeFns.stringRuntime());
    natives.add(new Native("$apply", 2, WasmNativeFns.applyEntry(dispatch)));
    natives.add(new Native("$recordGet", 2, WasmNativeFns.recordGetEntry()));
    natives.add(new Native("$recordSet", 3, WasmNativeFns.recordSetEntry()));
  ...
  // Binary-format plumbing lives in WasmEncoding (shared with WasmGc); these thin wrappers keep the
  // many unqualified call sites in this file unchanged.
  static void nameSection(ByteArrayOutputStream out, List<String> funcNames) {
    WasmEncoding.nameSection(out, funcNames);
  }

  static void nameSection(
      ByteArrayOutputStream out, List<String> funcNames, List<List<String>> localNames) {
    WasmEncoding.nameSection(out, funcNames, localNames);
  }
  ...
  static void nameSection(
    ...
    WasmEncoding.nameSection(out, funcNames, localNames, typeNames, fieldNames);
  }
